...
int MemoryManager::allocatePages(enum AddressPoolType type, const int count)
{
    ...
    bool success = true;  // 标记是否成功分配所有页面
    int physicalPageAddress;
    int vaddress = virtualAddress;
    int i = 0;

    // 依次为每一个虚拟页指定物理页
    for (; i < count && success; ++i, vaddress += PAGE_SIZE)
    {
        // 第二步：从物理地址池中分配一个物理页
        ...
        if (physicalPageAddress)
        {
            ...
            success = connectPhysicalVirtualPage(vaddress, physicalPageAddress);
            if (!success) {
                // 如果连接失败，释放刚刚分配的物理页
...
bool MemoryManager::connectPhysicalVirtualPage(const int virtualAddress, const int physicalPageAddress)
{
    ...
    if(!(*pde & 0x00000001)) 
    {
        ...
        *pde = page | 0x7;
        ...
        // 初始化页表 - 改进：获取整个页表的虚拟地址，并清零整个页表
        int *pageTable = (int *)(0xffc00000 | ((virtualAddress & 0xffc00000) >> 10));
    ...
    for (int i = 0; i < count; ++i, vaddr += PAGE_SIZE)
    {
        ...
        *pte = 0;
        ...
        if (*pde & 0x00000001) // 如果页表存在
        {
            ...
            int *pageTable = (int *)(0xffc00000 | ((vaddr & 0xffc00000) >> 10));
            pageTableEmpty = true;
            ...
            if (pageTableEmpty) 
            {
                releasePhysicalPages(AddressPoolType::KERNEL, pageTablePhysAddr, 1);
                *pde = 0;
            }
        }
    }
